...
import jujutsu.tsne.barneshut.AtomicDouble;
import jujutsu.tsne.barneshut.BHTSne;
import jujutsu.tsne.barneshut.DataPoint;
import jujutsu.tsne.barneshut.SPTree;
...
public class ParallelBHTsne extends BHTSne {
	...
	class RecursiveGradientCalculator extends RecursiveAction {
		...
		int endRow = -1;
		int limit = 100;
		jujutsu.tsne.barneshut.SPTree tree;
		double[][] neg_f;
		double theta;
		jujutsu.tsne.barneshut.AtomicDouble sum_Q;

		public RecursiveGradientCalculator(SPTree tree, double [][] neg_f , double theta,
                                           jujutsu.tsne.barneshut.AtomicDouble sum_Q, int startRow, int endRow, int ll) {
			this.limit = ll;
	...
	void computeGaussianPerplexity(double [] X, int N, int D, int [] _row_P, int [] _col_P, double [] _val_P, double perplexity, int K) {
		...
		ParallelVpTree<jujutsu.tsne.barneshut.DataPoint> tree = new ParallelVpTree<jujutsu.tsne.barneshut.DataPoint>(gradientPool,distance);
		final jujutsu.tsne.barneshut.DataPoint[] obj_X = new jujutsu.tsne.barneshut.DataPoint[N];
		for(int n = 0; n < N; n++) {
			double [] row = MatrixOps.extractRowFromFlatMatrix(X,n,D);
			obj_X[n] = new jujutsu.tsne.barneshut.DataPoint(D, n, row);
		}
		tree.create(obj_X);
		...
		List<Future<ParallelVpTree<jujutsu.tsne.barneshut.DataPoint>.ParallelTreeNode.TreeSearchResult>> results = tree.searchMultiple(tree, obj_X, K+1);

		for (Future<ParallelVpTree<jujutsu.tsne.barneshut.DataPoint>.ParallelTreeNode.TreeSearchResult> result : results) {
			ParallelVpTree<jujutsu.tsne.barneshut.DataPoint>.ParallelTreeNode.TreeSearchResult res = null;
